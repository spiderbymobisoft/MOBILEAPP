...
export class StreetPhotoPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,
    private us: UpdateService, private ss: SharedServices,
  ...
  save() {
    ...
    if (this.payload.photo) {
      this.geolocation.getCurrentPosition({ timeout: 30000, enableHighAccuracy: true }).then((position) => {
        ...
        this.us.uploadStreetPhoto(this.payload).subscribe(data => {
          if (data.success) {
            this.ss.toast('Street photo uploaded succesfully. You can upload another photo or go back.', 3000);
            this.payload.title = '';
            this.payload.photo = '';
